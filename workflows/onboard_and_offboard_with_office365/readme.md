...
# Onboard and Offboard with Microsoft 365

## Overview

This page outlines a sample workflow template that contains three workflows:

An **Microsoft 365 Staged Deactivation** flow: In this flow, once a user is removed from a specified Okta group:
* The users Microsoft 365 account will be disabled immediately.
* An email to the users manager to that effect will be sent.
* After a specified delay, the user's Microsoft 365 license will be removed.
* An email to the users manager to that effect will be sent.

An **Microsoft 365 Activation** flow: Once a user is added to a specified Okta group:
* The users Microsoft 365 account will be enabled immediately.
* The users previously assigned Microsoft 365 licenses will be reassigned immediately.
* An email to the users manager to that effect will be sent.

A Store **All Microsoft License** flow: This flow stores the current Microsoft subscription licenses in a Okta Workflows table called Microsoft 365 Licenses to be used in assigning the Microsoft license to the target user during the O365 Activation flow.

## Prerequisites

For this flow template in order to send an email to the manager on the creation of a user, an Microsoft 365 Mail connection and an Microsoft 365 Admin connection needs to be configured in addition to the Okta tenant connection. _You will need to have these connections created and authorized prior to installing the template.

You will also need Access to the Microsoft 365 Admin Portal as an admin user. [https://admin.microsoft.com/AdminPortal/](https://admin.microsoft.com/AdminPortal/)

## Setup Steps

### Microsoft 365 Setup Steps

Create a manager user in the Microsoft 365 tenant
...
Create a target user in the Microsoft 365 tenant
...
5. Type in Microsoft 365 Assignments for the group name and the description
...
3. Enter the following information based on information that you recorded above in the Create a target user in the Microsoft 365 tenant step. 
    ...
    * Enter Password: the desired password for this user.  (for ease of testing this Okta workflow, make it the same password you recorded for this user in the Create a target user in the Microsoft 365 tenant step.

4. Click the Save button

5. Enter the following information based on information that you recorded above in the Create a manager user in the Microsoft 365 tenant step.
    ...
    * Primary email: this should be the same as the Username field
    * Password: Select Set by admin
    * Enter Password: the desired password for this user.  (for ease of testing this Okta workflow, make it the same password you recorded for this user in the Create a manager user in the Microsoft 365 tenant step.
...
4. Go to Tables in the Microsoft 365 folder
...
9. Change the setup variables as desired:
    ...
    Delay Integer: The increment of time as specified by the Delay Interval Type that will transpire between the time the users Microsoft 365 account is disabled and the users Microsoft 365 licenses are removed.

    Delay Interval Type: The Delay Interval Type of time as specified by the Delay Integer that will transpire between the time the users Microsoft 365 account is disabled and the users Microsoft 365 licenses are removed. (Ex. if Delay Integer is set to 1 and Delay Interval Type is minute then there will be a delay of one minute.  The valid Delay Interval Types are: second, minute, hour, week, month, year.
...
2. Observe the current Microsoft 365 target user

3. Open and login to the Microsoft 365 [Admin Portal](https://admin.microsoft.com/AdminPortal/) as your Microsoft 365 admin user.

4. Go to Users->Active Users

5. Click on the username for the test user that is a member of your Okta group.

6. Observe that the user has an Microsoft 365 license assigned and is allowed to Sign In.

7. Deactivate the current Microsoft 365 User

8. Remove the user from the specified Okta group.

9. Open the email mailbox for the manager user.

10. You should see an email message that the user removed from the Okta group specified has been disabled in Microsoft 365.

11. If you return to the Microsoft Microsoft 365 Admin Portal and refresh the page, then select that user that the user is not allowed to sign in. The user still has an Microsoft 365 License assigned.
...
13. If you return to the Microsoft Microsoft 365 Admin Portal and refresh the page, sure enough the user targeted is unlicensed.

14. Activate the current Microsoft 365 User

15. In the Okta Workflows console, turn on the O365 Activation flow.

16. Now go to the Okta Admin console.

17. We will reset the users Microsoft 365 status. Put that same user back in the Okta specified group.

18. The manager will get an email that the user's Microsoft 365 user has been enabled and license restored.

19. Sure, enough if you return to the Microsoft Microsoft 365 Admin Portal and refresh the page, then click on the target user, you can see that the license has been restored and the user is allowed to sign in again.
...
The scope of this workflow does not include creating Microsoft 365 users, federating the Microsoft 365 tenant with Okta for Single Sign On or turning on Microsoft 365 provisioning with Okta.  
